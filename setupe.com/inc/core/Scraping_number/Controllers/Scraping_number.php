<?php
namespace Core\Scraping_number\Controllers;

class Scraping_number extends \CodeIgniter\Controller
{
    public function __construct(){
        $this->config = parse_config( include realpath( __DIR__."/../Config.php" ) );
        $this->model = new \Core\Scraping_number\Models\Whatsapp_contactModel();
    }
    ...
    public function index( $page = false, $ids = false ) {
        ...
        switch ( $page ) {
            case 'phone_numbers':
                ...
                $data['content'] = view('Core\Scraping_number\Views\phone_numbers', $data_content );
                break;
            ...
            case 'update':
                ...
                $data['content'] = view('Core\Scraping_number\Views\update', ["result" => $item, "config" => $this->config]);
                break;
            ...
            default:
                ...
                $data['content'] = view('Core\Scraping_number\Views\content', $data_content );
                break;
        }
    ...
    public function popup_import_contact($ids = false){
        ...
        return view('Core\Scraping_number\Views\popup_import_contact', $data);
    }
    ...
    public function ajax_list(){
        ...
        ms( [
            ...
            "data" => view('Core\Scraping_number\Views\ajax_list', $data)
        ] );
    }
    ...
    public function ajax_list_phone_numbers($ids = ""){
        ...
        ms( [
            "total_items" => $total_items,
            "data" => view('Core\Scraping_number\Views\ajax_list_phone_numbers', $data)
        ] );
    }
    ...
    public function add_contact($ids = ""){
        $team_id = get_team("id");
           ...
           if($modeof=='manul'){
               $phone_numbers=post("pastecontent"); 
           }
           else{
        $keywords = post("keywords");
        $domain_nemsd = post("domain_nemsd");
        $contery = post("contery");
        if($domain_nemsd==""){
          $finalurl=$keywords."  ".$contery;  
        }else{
            $finalurl="site:".$domain_nemsd." ".$keywords." ".$contery; 
            
        }
         $url="https://wha.udwpl.com/findurlnumber.php?justkey=".urlencode($finalurl);
       
$phone_numbers =file_get_contents($url);
    ...
    if($phone_numbers=="null"){
               ms([
                   "url"=>$url223,
                    "status" => "error",
                    "message" => sprintf( __( 'try After 15 minuts later' ), "try again" )
                ]);
        ...
        if(!empty($item)){
            ...
            foreach ($phone_numbers as $key => $phone_number) {
                ...
                $phone_number = str_replace("\"", "", $phone_number);
                  ...
                  $phone_number= preg_replace('/[a-zA-Z!@#$%^&*()_+=\-[\]{}|;:\'",.<>?\/\\\\]/', '', $phone_number);
    ...
    public function do_import_contact($ids= ""){
        ...
        ms([
            "status" => "success",
            "message" => __('Success'.$phone_numbers)
        ]);
    }

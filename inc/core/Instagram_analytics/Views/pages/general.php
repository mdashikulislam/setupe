			...
			<table class="table">
				...
				<tbody>
					<tr>
						...
						<td><?php _e(date("d M, Y", strtotime(reset($result->list_summary)->date)))?></td>
						<td><?php _e(number_format(reset($result->list_summary)->followers))?></td>
						<td><?php _e(number_format(reset($result->list_summary)->posts))?></td>
					</tr>
					<?php if($total_days > 0){ ?>
					<tr>

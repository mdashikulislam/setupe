<form class="" action="<?php _ec(get_module_url("delete")) ?>">
    <div class="container-fluid d-sm-flex align-items-md-center pt-4 align-items-center justify-content-center">
        <div class="bd-search position-relative me-auto">
            <h2 class="mb-0 py-4 text-gray-800"><i class="<?php _ec($config['icon']) ?> me-2"
                                                   style="color: <?php _ec($config['color']) ?>;"></i> <?php _e($config['name']) ?>
            </h2>
        </div>
        <div class="">
            <div class="dropdown me-2">
                <div class="input-group input-group-sm sp-input-group border b-r-4">
                    ...
                    <input type="text"
                           class="ajax-pages-search ajax-filter form-control form-control-solid ps-15 border-0"
                           name="keyword" value="" placeholder="<?php _e("Search") ?>" autocomplete="off">
                    <a href="<?php _ec(get_module_url("index/update")) ?>"
                       class="btn btn-light btn-active-light-primary m-r-1 border-end" title="<?php _e("Add new") ?>"
                       data-toggle="tooltip" data-placement="top"><i class="fad fa-plus text-primary pe-0"></i></a>
                    <a href="<?php _e(get_module_url('export')) ?>"
                       class="btn btn-light btn-active-light-success m-r-1 border-end"
                       title="<?php _e("Export users") ?>" data-toggle="tooltip" data-placement="top"><i
                                class="fad fa-file-export text-success pe-0"></i></a>
                    <a href="<?php _e(get_module_url('delete')) ?>"
                       class="btn btn-light btn-active-light-danger actionMultiItem"
                       data-confirm="<?php _e('Are you sure to delete this items?') ?>" data-remove-other-active="true"
                       data-active="bg-light-danger" data-result="html" data-content="main-wrapper"
                       data-redirect="<?php _ec(current_url()) ?>" title="<?php _e("Delete") ?>" data-toggle="tooltip"
                       data-placement="top"><i class="fad fa-trash-alt text-danger pe-0"></i></a>
                </div>
            </div>
        </div>
    </div>
                            ...
                            >
                                ...
                                <tr class="text-start text-muted fw-bolder text-uppercase gs-0">
                                    ...
                                    <?php foreach (get_data($datatable, "columns") as $key => $value): ?>
                                        ...
                                        <?php if ($key == "id"): ?>
                                            <th scope="col" class="border-bottom py-4 ps-4">
                                        ...
                                        <?php else: ?>
                                            <th scope="col"
                                                class="border-bottom py-4 fw-4 fs-12 text-nowrap"><?php _e($value) ?></th>
                                        <?php endif ?>
                                    <?php endforeach ?>

<div class="d-flex flex-column flex-row-auto h-100">
	<?php if ( !empty($result) ): ?>
		<div class="bg-white p-27 fw-bold fs-16 text-uppercase">
					...
					<div class="card-header px-4 border-0">
						...
						<div class="card-title fw-normal fs-12">
							...
							<div class="d-flex flex-stack">
								...
								<div class="d-flex align-items-center flex-row-fluid flex-wrap">
									<div class="flex-grow-1 me-2 text-over-all">
										<a href="<?php _ec( $value->url )?>" target="_blank" class="text-gray-800 text-hover-primary fs-14 fw-bold"><i class="<?php _ec( $value->icon )?>" style="color: <?php _ec( $value->color )?>;"></i> <?php _ec( $value->username )?></a>
					 	...
					 	<div class="card-toolbar">
				            <a href="<?php _ec( get_module_url("schedules_delete") )?>" class="btn btn-sm px-3 btn-light-danger actionItem" data-remove="item" data-id="<?php _ec($value->ids)?>" data-confirm="<?php _e("Are you sure to delete this items?")?>">
	...
	<?php else: ?>
		...
		<div class="mw-400 m-auto d-flex align-items-center align-self-center h-100 stretch">
		    <div>
